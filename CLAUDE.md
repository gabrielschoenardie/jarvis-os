...
- **`src/lib/brain-scene.js`** — imperative three.js scene (no React re-renders): `Points` + custom ShaderMaterial (per-node size ∝ √degree, additive blending, twinkle), `LineSegments` links that brighten on hover/selection, UnrealBloomPass + **OutputPass** (required for correct color space through EffectComposer), arc-reactor nucleus (pulses fast on `thinking`, ripple sphere while `speaking`), `d3-force-3d` simulation ticked inside rAF with an 8ms/frame budget (organic settle ~3-5s, then frozen; nodes pushed outside the r=22 nucleus zone). Camera focus tween on node select; click-vs-drag disambiguated by 5px displacement. Layout positions cached in `vault.layoutCacheRef` keyed by `scanId` so mode re-entry skips re-simulation. **Set the background via `scene.background` (managed `Color`), never `setClearColor` raw hex** — the raw clear color bypasses color management and OutputPass washes the near-black to gray-blue. Visual/perf constants live in the single `TUNING` block at the top of the file — no magic numbers below it. Node and link colors come from `DOMAIN_COLORS` (semantic hue per vault domain, `ACCENT` as fallback); the "`C.accent` is the sole protagonist" rule governs the HUD tokens in `constants.js`, not this 3D encoding. All assets procedural (COEP `require-corp` blocks external textures).
  - **Lifecycle contract** — `dispose()` is exhaustively symmetric *and* idempotent (`if (disposed) return`): StrictMode double-mounts in dev, and a second call must be a no-op. Every listener is a named function so it can actually be removed — an anonymous arrow on `controls`'s `'start'` event was unremovable. `EffectComposer.dispose()` frees only its render targets + copyPass, so `RenderPass`/`UnrealBloomPass`/`OutputPass` are each held in a variable and disposed explicitly. `renderer.forceContextLoss()` returns the GPU context immediately instead of waiting on GC — browsers cap ~16 live contexts and TERMINAL↔VAULT toggling would exhaust it. After dispose, `setGraph`/`setPulse`/`focusNode`/`clearFocus` are no-ops so a late callback can't revive the loop; `getPositions` stays live because `VaultBrain` calls it in cleanup *before* `dispose()`.
  - **WebGL context loss/restore** — `webglcontextlost` calls `preventDefault()` (authorizing the browser to restore) and fires `onContextLost`; the rAF stops on its own since `animate()` returns before rescheduling. `webglcontextrestored` **only signals**, via `onContextRestored` — it deliberately does not rebuild the resource graph or start a second loop. `VaultBrain` reacts by bumping `resetKey`, which routes through `dispose()` + a fresh `createBrainScene`. The REINICIAR button still covers the case where the restore never arrives.
  - **Adaptive quality** — the sampled signal is the *frame interval*, which is floored by vsync (~16.7ms at 60Hz), so the recovery threshold has to sit above that floor: `FRAME_BUDGET_MS * 0.9` is reachable, while the earlier `* 0.6` (12ms) never was and pinned a degraded scene at low resolution forever. Hysteresis comes from the asymmetric counts (30 frames down / 120 up) and the reset on any contrary frame — not from a tighter band, which is what keeps the idle throttle from making quality oscillate. Resolution changes go through `composer.setPixelRatio`, never `setSize` (bloom would silently stop being full-res). The idle throttle uses `IDLE_INTERVAL_MS` (`1000/IDLE_FPS` minus ~¼ frame); without that tolerance the 33.3ms budget lands on the 3rd rAF tick and settles at ~20fps instead of 30.
  - **Hover label** — owned by the scene (a `<div>` appended to the container, not React state). Positioned *after* `controls.update()`/`composer.render()` so it uses the frame's own camera matrix and tracks the node during autoRotate; `screenPos` reads `renderer.getSize()` rather than `getBoundingClientRect()`, since a per-frame rect read right after writing `style.left/top` is layout thrash.
...
- **`src/lib/chunker.js`** — pure: `chunkText(rawText)` strips frontmatter and splits the body into ~900-char chunks with ~150-char overlap, preferring a paragraph (`\n\n`) or heading (`#`) break in the final half of each window over a hard cut. `chunkNote(rawText, { path, title })` (Fase A.1) wraps the *same* cut — both delegate to one internal `cutChunks`, so `chunkNote(t,{}).map(c => c.text)` is byte-identical to `chunkText(t)` (there's a test pinning this) — and adds `{ heading, headingPath, chunkIndex, embeddingText }`. Heading attribution is by **dominant coverage**, not by the chunk's start offset: since the cut prefers heading boundaries and the overlap then rewinds 150 chars, a start-offset rule would label almost every chunk with the *previous* section. Headings inside ``` fences don't count.
- **`src/lib/vectorIndex.js`** — pure: `diffIndex(graph, index)` diffs the current graph against the persisted index by `mtime` to find notes needing (re)embedding vs. removal; `packVectors`/`unpackVector` pack per-chunk `Float32Array`s into one contiguous buffer; `search(index, queryVec, k)` is brute-force dot product (vectors are pre-normalized, so dot product == cosine) — fast enough in JS at vault scale, no ANN needed. `search` returns the chunk's structural metadata (`title`/`heading`/`headingPath`/`chunkIndex`) alongside `path`/`text`/`score` — additive, so old-format chunks just yield `undefined`. `isIndexCompatible(saved, { version, model, dims })` is the load-time guard, extracted here (rather than inline in the hook) purely so it's reachable from `node --test`.
...
- **`src/hooks/useVaultIndex.js`** — orchestrates incremental indexing (diffed by `mtime` against the previous scan, batched embedding of only changed notes) and hybrid search (`0.75×cosine + 0.25×recency`, 30-day half-life, best chunk per note deduped). Persists the packed index to IndexedDB (`jarvis-vault-index`). An embed failure mid-loop is caught and falls back to `indexStatus: 'unavailable'` rather than leaving the UI stuck on "indexing" — same handling as a `warmup()` failure. `INDEX_VERSION` is **2** since Fase A.1 (the chunk record gained structural fields); the load guard checks `version`/`model`/`dims`, so a v1 index is discarded and rebuilt once. What gets embedded is the chunk's `embeddingText` (title + `Seção: headingPath` + body, so the vector carries structural context); what gets **persisted** is only the display `text` — `embeddingText` is derivable and would double the index size. The `passage:`/`query:` prefix is still applied once, in the worker.
- **`src/lib/memoryContext.js` — context assembly (Fase A.1)**: `buildRelevantExcerpt(text, maxChars)` replaced a blind `slice(0, 400)`. The chunk that won semantically is often ~900 chars whose signal sits mid- or end-of-chunk, so taking the head threw away the very reason it was retrieved; it now returns the whole body when it fits, else a **composite head+tail window** (60/40, snapped to paragraph → sentence → word boundaries, joined by `' … '`). It never fakes a semantic location inside the chunk — E5 gives no token offsets. `selectParts` allocates the 2000-char budget in **two passes**: pass 1 gives every entry up to `MAX_EXCERPT_CHARS` (400) so breadth matches the old behavior, then pass 2 redistributes the leftover in score order, expanding truncated entries toward their full text. So 400 is now a *floor*, not a ceiling — a 600-char chunk arrives whole when there's room, and four 900-char chunks land at ~802/388/388/388 (1990 total) instead of four blind 400s. `buildMemoryContext(entries, { mode })` labels semantic retrieval `'Contexto relevante recuperado do vault:'` vs. the recency fallback's `'Notas recentes do vault:'`; both args are optional, so the old one-arg call still works.
- **Memory trace (Fase A.1)**: `useVault` keeps a `memoryTrace` **ref** (not state — `searchMemory` runs on every chat message and this must not re-render `App`) holding `{ at, mode, query, candidates, selected[], totalChars, totalTokens }`. `selected` is an explicit metadata allowlist (`path`/`title`/`score`/`chunkIndex`/`chunkCount`/`chars`/`tokens`) — **never `excerpt` or `content`**. In-memory only: never persisted, never sent, never `console.log`ged. Currently has no consumer by design; it exists for a future debug surface.
