...
- **Conectar**: clique em `▸ CONECTAR VAULT` e escolha a pasta do seu vault. As notas são lidas **inteiramente no navegador**. Conteúdo sobe a servidor em dois casos: quando você clica em `▸ ANALISAR COM JARVIS` numa nota específica (só o texto daquela nota), e automaticamente, em toda mensagem do chat, um resumo curto das notas mais recentemente modificadas do vault (memória de curto prazo — indicador `🧠 MEMÓRIA` na tela quando ativo; ver "Memória do vault" abaixo).
