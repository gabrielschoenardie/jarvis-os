...
>
> **4b — Polimento ✅ CONCLUÍDA (parcial)**: ErrorBoundary no VaultBrain (M3); AIText++ — código visível durante o stream, links `[x](url)`+URLs cruas, `###`, formatação inline em list items (M1); void 2-stop (`radial #071018→#050a14→#030710`, tokens `C.bgSoft/bgDeep`); subsistema VAULT do rail reflete `vault.status` (H9, leve — resto é lore Stark); numerais tabulares nos números vivos (latência/tokens/turnos/relógio); HudMediaWindow arrastável pela barra + vinheta de backdrop (M4); VoicePanel com divisores entre clusters (M5, leve). Build OK (188 módulos).
>
> **4b — deixado pro olho do usuário (H5)**: retune agressivo de contraste/tipografia (elevar `C.dim` global a ~3:1, Rajdhani só display, revisão de tracking) — subjetivo e não verificável headless; command input "cockpit" e VoicePanel em clusters completos também ficaram pra uma passada guiada.
