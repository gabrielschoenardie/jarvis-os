...
export function VoiceIndicator({ voiceOut, speaking, listening, onToggle, onPanel, supported }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <button onClick={onPanel} style={{ background: 'transparent', border: `1px solid ${C.line}`, color: C.muted, padding: '2px 6px', fontFamily: 'inherit', fontSize: 10, letterSpacing: '0.2em', cursor: 'pointer' }}>◇</button>
    </div>
  );
}
...
export function ToggleBtn({ label, on, onClick, disabled }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: disabled ? 0.4 : 1 }}>
      <span style={{ fontSize: 10, letterSpacing: '0.28em', color: C.muted }}>{label}</span>
